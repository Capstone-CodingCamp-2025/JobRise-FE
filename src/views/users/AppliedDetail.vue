<template>
  <div class="py-6 px-4 md:py-10 md:px-16 relative">
    <router-link
      :to="{ name: 'applied' }"
      class="absolute right-4 top-2 md:right-10 md:top-2 z-10"
    >
      <Icon
    ...
    <div v-else-if="error" class="text-center py-10 min-h-[calc(100vh-200px)]">
      ...
      <button
        @click="initializeApplicationPageData"
    ...
    <div v-else-if="applicationDetail">
      ...
      >
        ...
        >
          <div
            class="w-16 h-16 md:w-28 md:h-28 bg-slate-200 rounded-sm flex-shrink-0 flex items-center justify-center overflow-hidden"
          >
            <img
              v-if="applicationDetail.company_logo"
              :src="
                applicationDetail.company_logo.startsWith('http')
                  ? applicationDetail.company_logo
                  : `${baseImageUrl}${applicationDetail.company_logo}`
              "
            ...
            />
            <span
              v-if="isLogoError || !applicationDetail.company_logo"
          ...
          <div>
            <p class="font-bold text-base md:text-xl">
              {{ formatTitle(applicationDetail.title) }}
            </p>
            <p class="font-medium text-gray-600 text-xs md:text-base">
              {{
                formatTitle(applicationDetail.company_name) ||
                "Company Name N/A"
              }}
            </p>
          </div>
        ...
        <div class="flex pt-2 md:pt-5 gap-x-2 md:gap-x-4 items-center">
          <button
            ...
            :disabled="
              jobStore.isCreatingFavorite ||
              jobStore.isDeletingFavorite ||
              !applicationDetail.job_id
            "
          ...
          <router-link
            :to="{ name: 'find-job' }"
            class="bg-blue-950/80 text-white w-36 md:w-44 py-1.5 md:pt-2 text-center h-8 md:h-10 relative rounded-md pr-4 md:pr-6 text-xs md:text-sm flex items-center justify-center"
          >
            Explore More Jobs
            <Icon
        ...
        >
          ...
          >
            ...
            <p class="text-green-500 font-medium text-xs md:text-base">
              {{ formatGajiRingkas(applicationDetail.salary_min) }} -
              {{ formatGajiRingkas(applicationDetail.salary_max) }}
            </p>
          </div>
          <div
            class="px-4 py-3 md:px-10 md:py-8 w-full border-b border-t border-r border-slate-300"
          >
            <p class="font-semibold text-xl md:text-3xl">Location</p>
            <p class="text-gray-600 font-medium text-xs md:text-base">
              {{ formatTitle(applicationDetail.location) || "N/A" }}
            </p>
          </div>
          <div
            class="px-4 py-3 md:px-10 md:py-8 w-full border-b border-t border-r border-slate-300"
      ...
      <div class="my-6 md:my-0">
        ...
        <p
          v-else
          class="text-justify text-gray-500 text-xs md:text-base leading-relaxed"
        >
      ...
      <div class="py-16 md:py-28">
        <div
          class="bg-blue-200/40 w-fit m-auto flex gap-x-4 md:gap-x-10 px-6 py-3 rounded-md items-center"
        >
          <p class="text-blue-800 font-bold text-xl md:text-2xl">
            Status Apply
          </p>
          <span
            :class="
              getStatusVisual(applicationDetail.status).bgClass +
              ' ' +
              getStatusVisual(applicationDetail.status).textClass
            "
...
const isLoading = computed(
  () => jobStore.loadingApplicationDetail || jobStore.loadingUserFavoriteJobs
); // Loading gabungan
const error = computed(
  () => jobStore.errorApplicationDetail || jobStore.errorUserFavoriteJobs
); // Error gabungan
...
const isCurrentJobFavorited = computed(() => {
  if (
    ...
    !applicationDetail.value.job_id ||
    !jobStore.userFavoriteJobIds
  ) {
    return false;
  }
  return jobStore.userFavoriteJobIds.has(
    parseInt(applicationDetail.value.job_id)
  );
});
...
const formatSalary = (min, max) => {
  if (
    min &&
    max &&
    String(min).toLowerCase() !== "null" &&
    String(max).toLowerCase() !== "null" &&
    min !== "" &&
    max !== ""
  ) {
    return `Rp ${min} - ${max}`;
  }
  if (min && String(min).toLowerCase() !== "null" && min !== "")
    return `Rp ${min}`;
  if (max && String(max).toLowerCase() !== "null" && max !== "")
    return `Hingga Rp ${max}`;
  return "N/A";
};
...
const initializeApplicationPageData = async () => {
  ...
  if (!authStore.tokenUser) {
    ...
    jobStore.errorApplicationDetail = {
  ...
  try {
    ...
    await Promise.all([
      jobStore.fetchApplicationDetailById(
      ...
      jobStore.userFavoriteJobIds.size === 0
  ...
  if (isCurrentJobFavorited.value) {
    ...
    try {
      ...
      if (
        error.response &&
        error.response.data &&
        error.response.data.message === "Job already favorited"
      ) {
        if (
          !jobStore.userFavoriteJobIds.has(jobIdToToggle) &&
          authStore.tokenUser
        ) {
