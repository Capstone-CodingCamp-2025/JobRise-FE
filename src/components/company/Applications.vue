        ...
        <div class="overflow-y-auto">
          ...
          >
            ...
            <header class="text-center mb-8">
              ...
              <div
                class="mt-2 text-sm text-gray-600 flex justify-center items-center gap-x-3 md:gap-x-4 flex-wrap"
              >
                <a
                  v-if="cv.email"
                ...
                <span v-if="cv.phone && cv.city" class="hidden md:inline"
                  >|</span
                >
                <span v-if="cv.city" class="flex items-center gap-1">
                  ...
                  >
                    <path
                      fill="currentColor"
                      d="M12 12q.825 0 1.413-.588T14 10q0-.825-.588-1.413T12 8q-.825 0-1.413.588T10 10q0 .825.588 1.413T12 12m0 7.5q2.5-2.25 4.25-4.375T18 12.15q0-2.225-1.588-3.813T12 6.75q-2.825 0-4.413 1.588T6 12.15q0 1.925 1.75 4.05T12 19.5"
                    />
                  </svg>
                  {{ cv.city }}
                </span>
                <span
                  v-if="cv.city && (cv.linkedin || cv.portofolio_url)"
            ...
            <main class="space-y-8">
              ...
              <section v-if="cv.educations && cv.educations.length > 0">
                ...
                <div class="space-y-6">
                  <div v-for="edu in cv.educations" :key="edu.id">
                    ...
                    <p v-if="edu.gpa" class="text-sm text-gray-500 mt-1">
                      GPA: {{ edu.gpa }}
                    </p>
                    <p v-if="edu.description" class="mt-2 text-gray-700">
                      {{ edu.description }}
                    </p>
                  </div>
                </div>
              ...
              <section v-if="cv.skills && cv.skills.length > 0">
                ...
                >
                  ...
                  <span v-if="skill.level" class="font-normal opacity-75">
                    ({{ skill.level }})</span
                  >
                </div>
              </section>
                ...
                <div class="space-y-6">
                  <div v-for="project in cv.projects" :key="project.id">
                    <div class="flex justify-between items-baseline flex-wrap">
                      <h3 class="text-md font-semibold text-gray-900">
                        <a
                          v-if="project.link_url && project.link_url !== '...'"
              ...
              <section v-if="cv.certifications && cv.certifications.length > 0">
                ...
                <div class="space-y-6">
                  <div v-for="cert in cv.certifications" :key="cert.id">
                    ...
                    <p
                      v-if="cert.id_credential_url"
                      class="text-sm text-gray-500 mt-1"
                    >
                      Credential ID: {{ cert.id_credential_url }}
                    </p>
                    <p v-if="cert.description" class="mt-2 text-gray-700">
                      {{ cert.description }}
                    </p>
                  </div>
                </div>
...
// 1. Impor 'useRoute' dari vue-router
...
// 2. Dapatkan instance dari route
...
// 3. Ubah 'currentJobId' menjadi computed property yang dinamis
//    Pastikan nama parameter di route Anda adalah 'id'. Jika berbeda (misal: 'jobId'),
//    sesuaikan menjadi route.params.jobId
...
// Watcher ini akan otomatis terpanggil saat Anda berpindah halaman
// antar job yang berbeda, karena `currentJobId` (computed) akan berubah.
...
const loadJobApplications = async () => {
  ...
  // Pastikan currentJobId.value ada sebelum fetch
  if (!currentJobId.value) {
    console.error("Job ID is missing from the route.");
    return;
  }
  try {
