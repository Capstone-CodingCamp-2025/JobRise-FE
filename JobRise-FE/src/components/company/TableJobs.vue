<template>
  <div class="px-4 pb-10 sm:px-8">
    ...
    <div v-if="jobsStore.isLoading" class="text-center py-8 bg-blue-50 rounded-md shadow-sm">
      <p class="text-blue-800 font-semibold text-lg">Memuat daftar pekerjaan...</p>
    ...
    <div v-else-if="jobsStore.error" class="text-center py-8 bg-red-50 rounded-md shadow-sm">
      <p class="text-red-600 font-semibold text-lg">Terjadi kesalahan saat memuat pekerjaan:</p>
      <p class="text-red-500 mt-2">{{ jobsStore.error }}</p>
    </div>

    <div v-else-if="displayedJobs.length === 0" class="text-center py-8 bg-yellow-50 rounded-md shadow-sm">
      <p class="text-yellow-700 font-semibold text-lg">
        {{ searchQuery ? 'Tidak ada pekerjaan yang cocok dengan pencarian Anda.' : 'Belum ada pekerjaan yang diposting.' }}
      </p>
      <p v-if="!searchQuery" class="text-yellow-600 mt-2">Mulai posting pekerjaan pertama Anda!</p>
    </div>
      ...
      <div class="overflow-x-auto">
        <table class="table-auto text-center w-full md:px-10 min-w-[600px]">
          <thead class="bg-slate-200 text-center">
            <tr>
              <th class="min-w-[200px] py-2 md:py-3 rounded-l-lg font-normal text-left pl-4 md:pl-20">Jobs</th>
              <th class="min-w-[150px] py-2 md:py-3 font-normal">Applications</th>
              <th class="min-w-[150px] py-2 md:py-3 font-normal">Status</th>
              <th class="min-w-[180px] py-2 md:py-3 rounded-r-lg font-normal">Action</th>
            </tr>
          </thead>
          <tbody>
            <tr
    ...
    <div class="flex justify-center mt-4" v-if="jobsStore.totalCompanyPages > 1">
      <button
        :disabled="jobsStore.currentCompanyPage === 1 || jobsStore.isLoading"
        @click="jobsStore.fetchCompanyJobs(jobsStore.currentCompanyPage - 1, 10, searchQuery)"
      ...
      <template v-if="jobsStore.totalCompanyPages <= 5">
        <button
          ...
          :class="['px-3 py-1', {'font-semibold bg-blue-200 rounded-md': jobsStore.currentCompanyPage === page}]"
        >
          {{ page }}
        </button>
      </template>
      <template v-else>
        <button
          ...
          :class="['px-3 py-1', {'font-semibold bg-blue-200 rounded-md': jobsStore.currentCompanyPage === 1}]"
        >
          1
        </button>
        <span v-if="jobsStore.currentCompanyPage > 3" class="px-1 py-1">...</span>
        <button
        ...
        <span v-if="jobsStore.currentCompanyPage < jobsStore.totalCompanyPages - 2" class="px-1 py-1">...</span>
        <button
          @click="jobsStore.fetchCompanyJobs(jobsStore.totalCompanyPages, 10, searchQuery)"
          :class="['px-3 py-1', {'font-semibold bg-blue-200 rounded-md': jobsStore.currentCompanyPage === jobsStore.totalCompanyPages}]"
        >
      ...
      <button
        :disabled="jobsStore.currentCompanyPage === jobsStore.totalCompanyPages || jobsStore.isLoading"
        @click="jobsStore.fetchCompanyJobs(jobsStore.currentCompanyPage + 1, 10, searchQuery)"
...
// Sekarang ini hanya akan mengembalikan data dari store, karena filtering akan dilakukan di backend.
...
const visiblePageNumbers = computed(() => {
  ...
  if (totalPages <= 5) {
    ...
    if (currentPageNum <= visibleRange + 1) {
      ...
      for (let i = currentPageNum - visibleRange; i <= currentPageNum + visibleRange; i++) {
        pages.push(i);
      }
    }
  }
...
onMounted(() => {
  jobsStore.fetchCompanyJobs(1, 10, searchQuery.value); // Panggil dengan parameter pencarian awal
});

// Watch searchQuery dan ambil data baru dari backend
watch(searchQuery, (newQuery) => {
  // Selalu kembali ke halaman 1 saat query pencarian berubah
  jobsStore.fetchCompanyJobs(1, 10, newQuery);
});
</script>
